...
describe('FlagZoomOverlay', () => {
  it('renders the flag image with correct src', () => {
    render(
      <FlagZoomOverlay src="fr.svg" countryName="France" onClose={vi.fn()} />
    );
  ...
  it('renders the flag image with correct alt text', () => {
    render(
      <FlagZoomOverlay src="jp.svg" countryName="Japan" onClose={vi.fn()} />
    );
    expect(screen.getByRole('img')).toHaveAttribute(
      'alt',
      'Zoomed flag of Japan'
    );
  });
  ...
  it('calls onClose when the overlay backdrop is clicked', () => {
    const onClose = vi.fn();
    render(
      <FlagZoomOverlay src="de.svg" countryName="Germany" onClose={onClose} />
    );
